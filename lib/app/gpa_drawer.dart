...
class GpaDrawer extends StatefulWidget {
  ...
  var _changeGradeType;

  GpaDrawer(
    Key key,
    this._removeClassList,
    this._changeGradeType,
  ) : super(key: key) {
    _gradeListType = ["A+", "AA"];
  }
...
class _GpaDrawerState extends State<GpaDrawer> {
  ...
  _loadGradeTypes() async {
    ...
    setState(() {
      widget._selectedGradeType = (prefs.getString(GpaDrawer.keySelectedGradeType) ??
    ...
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            shape: RoundedRectangleBorder(
                borderRadius:
                    BorderRadius.all(Radius.circular(widget._borderRadius))),
            title: Text("Uyarı"),
            content: Text(
